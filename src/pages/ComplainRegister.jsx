...
const mockComplaints = [
  { id: 'FB-1029', subject: 'Broken AC in Floor 3', priority: 'High', date: '2026-05-20', status: 'Open', description: 'The AC in the third floor near the engineering bay has been leaking water and is not cooling properly.' },
  { id: 'FB-1028', subject: 'Need better coffee in breakroom', priority: 'Low', date: '2026-05-19', status: 'Resolved', description: 'The current coffee brand is not great. Can we switch to a better espresso blend?' },
  { id: 'FB-1027', subject: 'Project management tool license expired', priority: 'Critical', date: '2026-05-18', status: 'In Progress', description: 'Our team cannot access the main project board. Need immediate renewal of the enterprise license.' },
  { id: 'FB-1026', subject: 'Unprofessional behavior in meeting', priority: 'Medium', date: '2026-05-15', status: 'Open', description: 'During the sprint planning, someone kept interrupting. Please enforce meeting guidelines.' },
];
      ...
      <div style={{ padding: '24px' }}>
        <div className="card" style={{ overflow: 'hidden' }}>
          <table className="emp-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                <th style={{ padding: '16px', textAlign: 'left', borderBottom: '1px solid var(--border-color)', fontSize: '12px', color: 'var(--text-light)', textTransform: 'uppercase' }}>Task ID</th>
            ...
            <tbody>
              {filtered.map(item => (
                <tr key={item.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                  <td style={{ padding: '16px', fontWeight: 600, color: 'var(--text-main)', fontSize: '13px' }}>{item.id}</td>
          ...
          {filtered.length === 0 && (
            <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-light)' }}>
              ...
              <div style={{ fontWeight: 600 }}>No feedback records found.</div>
            </div>
          )}
        </div>
      </div>
