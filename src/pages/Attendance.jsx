...
export default function Attendance({ onBack, employees = [] }) {
  ...
  // Helper: Deterministic statuses per employee per day number
  const getEmployeeStatusForDay = (emp, dayNum) => {
    if ((emp.id + dayNum) % 15 === 0) return 'On Leave';
    if ((emp.id + dayNum) % 22 === 0) return 'Permission';
    if ((emp.id + dayNum) % 12 === 0) return 'Late';
    return 'Present';
  };

  // Generate logs for selected day — use API data if available, else mock
  const dailyLogs = React.useMemo(() => {
    if (apiLogs.length > 0) {
      // Map API fields to the shape the original UI expects
  ...
  }, [activeEmployees, selectedDay, apiLogs]);
  ...
  const attStats = React.useMemo(() => [
    { label: 'Present', value: stats.present, trend: `May ${selectedDay < 10 ? `0${selectedDay}` : selectedDay}`, type: 'up', color: '#4CAA17', Icon: UserCheck },
    { label: 'On Leave', value: stats.leave, trend: `May ${selectedDay < 10 ? `0${selectedDay}` : selectedDay}`, type: 'down', color: '#FC8181', Icon: CalendarOff },
    { label: 'Permission', value: stats.permission, trend: `May ${selectedDay < 10 ? `0${selectedDay}` : selectedDay}`, type: 'up', color: '#9F7AEA', Icon: Clock },
    { label: 'Late', value: stats.late, trend: `May ${selectedDay < 10 ? `0${selectedDay}` : selectedDay}`, type: 'up', color: '#ECC94B', Icon: AlertTriangle },
  ], [stats, selectedDay]);
              ...
              {calendarGrid.map((day, idx) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => {
                      setSelectedDay(day);
                      showNotification(`Loading logs for May ${day < 10 ? `0${day}` : day}, 2024`, "info");
                    }}
                    style={isSelected ? {
            ...
            <div style={{ overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                {[
                  ...
                  { label: 'Date', value: `May ${selectedDay < 10 ? `0${selectedDay}` : selectedDay}`, color: '#ECC94B', bg: '#FFFBEB', border: '#FDE68A' },
                ].map(item => (
              ...
              <div style={{ background: 'white', border: '1px solid var(--border-color)', borderRadius: '8px', padding: '12px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                  {[
                    { label: 'Present', value: Math.floor(20 - (selectedLog.id % 4)), color: '#4CAA17' },
                    { label: 'Late', value: selectedLog.id % 3, color: '#ECC94B' },
                    { label: 'On Leave', value: selectedLog.id % 2 + 1, color: '#FC8181' },
                    { label: 'Permission', value: selectedLog.id % 2, color: '#9F7AEA' },
                  ].map(item => (
                    <div key={item.label}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button onClick={() => setSelectedLog(null)} style={{ flex: 1, padding: '8px', background: 'var(--bg-main)', border: '1px solid var(--border-color)', borderRadius: '7px', fontSize: '11px', fontWeight: 700, cursor: 'pointer', color: 'var(--text-muted)' }}>Clear</button>
                <button onClick={() => showNotification(`Report generated for ${selectedLog.name}`, 'success')} style={{ flex: 1, padding: '8px', background: 'var(--primary)', border: 'none', borderRadius: '7px', fontSize: '11px', fontWeight: 700, cursor: 'pointer', color: 'white' }}>Export</button>
              </div>
            </div>
          )}
        </div>
          ...
          <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
            <div>
              <h3 style={{ margin: 0, fontSize: '15px', fontWeight: 800, color: 'var(--text-main)' }}>
                Daily Logs: May {selectedDay < 10 ? `0${selectedDay}` : selectedDay}, 2024
              </h3>
                  ...
                  <tr key={log.id} style={{ cursor: 'pointer' }} onClick={() => setSelectedLog(log)}>
                    <td style={{ verticalAlign: 'middle' }}>
                      <div className="emp-table-user">
                        <div className="emp-table-avatar" style={{ background: log.color + '15', color: log.color, width: '32px', height: '32px', fontSize: '11px', flexShrink: 0 }}>
                          {log.initials}
                        </div>
                        <div style={{ minWidth: 0 }}>
                          <div className="emp-table-name" style={{ color: 'var(--primary)', cursor: 'pointer', textDecoration: 'underline', textUnderlineOffset: '2px', textDecorationStyle: 'dotted', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{log.name}</div>
                          <div className="emp-table-role" style={{ fontSize: '10.5px', color: 'var(--text-light)', marginTop: '1px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                            {log.employeeId || `EMP-100${log.id}`} • {log.role}
                          </div>
                        </div>
                      </div>
                    </td>
                    <td style={{ verticalAlign: 'middle', whiteSpace: 'nowrap' }}><div className="emp-table-email" style={{ fontWeight: 600, fontSize: '12px' }}>{log.checkIn}</div></td>
                    <td style={{ verticalAlign: 'middle', whiteSpace: 'nowrap' }}><div className="emp-table-email" style={{ fontWeight: 600, fontSize: '12px' }}>{log.checkOut}</div></td>
                    <td style={{ verticalAlign: 'middle', whiteSpace: 'nowrap' }}><div className="emp-table-email" style={{ fontWeight: 600, fontSize: '12px' }}>{log.workHours}</div></td>
                    <td style={{ verticalAlign: 'middle', whiteSpace: 'nowrap' }}>
                      <span style={{
                        ...
                        ...(log.status === 'Present' ? { background: '#F1F9EE', color: '#4CAA17', border: '1px solid #C2E7B0' } :
                            log.status === 'Late' ? { background: '#FEF3C7', color: '#D97706', border: '1px solid #FDE68A' } :
                            log.status === 'On Leave' ? { background: '#FFF5F5', color: '#FC8181', border: '1px solid #FED7D7' } :
                            { background: '#F5F3FF', color: '#9F7AEA', border: '1px solid #DDD6FE' })
                      }}>
                        {log.status}
                      </span>
                    </td>
                ...
                {displayLogs.length === 0 && (
                  <tr>
                    <td colSpan="5" style={{ textAlign: 'center', padding: '30px', color: 'var(--text-light)' }}>
                      No matching employee records found for this selection.
                    </td>
                  </tr>
                )}
              </tbody>
